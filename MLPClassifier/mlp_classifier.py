...
def sym_cross_entropy(Ybatch_hat, Ybatch):
    #import pdb; pdb.set_trace()
    return -T.mean(T.log(Ybatch_hat))
    #return -T.mean(T.log(Ybatch_hat) * Ybatch)
...
class MLPClassifier(object):
    ...
    def __init__(self, dims, activations, 
        ...
        if loss == 'cross_entropy':
            self.sym_cost_traintime = sym_cross_entropy(self.output_for_sym_Xbatch_traintime, self.sym_Ybatch)
            self.cost = sym_cross_entropy
    ...
    def output_given_input_evaluation(self, X, Ws, bs):
        ...
        for W, b, activation in zip(Ws, bs, self.activations):
            ...
            elif current_layer !=  output_layer:
                X = activation(T.dot(X, W) + b)
            elif current_layer ==  output_layer:
                X = activation(X)
